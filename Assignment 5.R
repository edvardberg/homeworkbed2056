...
dframe <- dframe %>% mutate (region = dplyr::recode(region, "Viken"="Viken", "Oslo" = "Oslo", "Innlandet" = "Innlandet", "Vestfold og Telemark" = "Vestfold_og_Telemark","Nordland" = "Nordland", "Svalbard"= "Svalbard","Møre og Romsdal" = "More_og_Romsdal","Trønderlag - Trööndelage" = "Trondelag", "Vestland" = "Vestland", "Troms og Finmark - Romsa ja Finmárku"= "Troms_og_Fimark","Rogaland" = "Rogaland", "Hele landet"="Whole_country" ))
...
x <- data.frame(dframe$region, dframe$value, dframe$variable3, dframe$date)
x <- subset(x, dframe$variable3 == "roomcap")

print(ggplot(x,aes(x=dframe.date, y=dframe.value, color=dframe.region))+ geom_line()+labs(title= "Roomcap over time", y= "roomcap", x= "Time", color="Locations"))
